...
**A present result file with `Status: FAILED` / `ABORTED` is NOT a spawn failure.** It means the subagent ran to completion and is reporting its own error — Step 2's `case` block owns that path. The fallback fires only when the file is **absent**, or the `Agent` call itself errored.
...
**Inline execution.** In the current session, run the `agents/dev/align-subagent.md` contract verbatim against the same inputs (`receipt_path = .dev/figma-context.md`, `change_dir = $CHANGE_DIR`, `ticket_id = $TICKET_ID`): extract cited node IDs, run the per-node citation + token-grounding (paragraph-scoped verb) checks, decide CLEAR vs CONFLICT, and atomic-write `.dev/align-result.md` in the **exact AGENTS.md Result file contract** (`Status:` / `Outputs:` / `Summary:` / `Data:`). On CONFLICT, also write `claude-reports/<ticket_id>/figma-alignment.md` with the per-node conflict list, identical to the subagent path — including the result file's `Data:` line (`{"nodes_failed":N,"nodes_total":M}`, per `agents/dev/align-subagent.md`), so an inline-produced CONFLICT carries the same machine-readable payload as a subagent-produced one.

**Provenance — trailing comment-line, never in `Status:`/`Summary:`.** After the contract block, append `# Provenance: inline-fallback` as the file's last line. This is verified inert: Step 2's `grep -m1 '^Status:'` and the walker's `grep -q '^Status: CLEAR'` (`ff.md`) both anchor on the first matching line and never see a trailing comment. Do **not** fold provenance into `Summary:` (the ≤200-char contract that is surfaced to the user verbatim) or `Status:`. (Deliberately different from figma's marker: figma's receipt is free-form markdown, so its provenance sits on **line 2 bare-prefix** — see `figma.md` Step 4b; this file is a strict line-anchored contract, so a trailing `#` comment is the only position guaranteed inert. If you ever touch one format, check the other.)

**Re-verify, then proceed — with an independent post-check.** After the inline write, re-run the Step 2 `Status:` parse on the file you just wrote, then branch on its `Status` exactly as the spawn path would (CLEAR → Step 4; CONFLICT → Step 3). Because align has no downstream equivalent of figma's Step 5 gate (the inline writer would otherwise be checking only its own output), add one mechanical cross-check before branching: grep every node ID cited in the result you just wrote against `### <fileKey>:<nodeId>` sections actually present in `.dev/figma-context.md`. Any cited node ID with no matching receipt section → treat the inline result as illegal (same as a malformed file): save the context to `claude-reports/<ticket_id>/subagent-malformed-align.md` and STOP.
