...
**Usage**: `/ggx-on-duty [--team:<KEY>] [--no-dispatch] [--no-analyze] [--workflow] [--until:HH:MM]`
...
- `--workflow` — **opt-in (D22).** Leg 1's dispatch phase runs as a top-level `Workflow` task **owned by this on-duty session** (`/ggx-dispatcher --workflow --launch-only`) instead of being chained inside the headless `claude -p` child — so the per-ticket fan-out is visible LIVE in your `/workflows` view. Default OFF (classic headless chain, D21). See Leg 1 → "Leg 1 invocation decision (D22)".
...
```
   ...
   - `chain` / `health`: background-agent liveness + **time-based** due timestamps (dynamic mode has no fixed tick to take a modulus of). Every spawn records the harness task id into `task_id` so RECONCILE can verify liveness via TaskList. `chain.phase` (default-path `"chain"` | `--workflow` `"analyze"`→`"dispatch"` | `null` when idle) drives the completion-notification branch in Leg 1; it is absent on `health` (health has no sub-phases). Older state files without `phase` load as `null` (the on-invocation merge fills it) — treat a `running=true` chain with `phase=null` as classic `"chain"` for back-compat.
...
- Else **start the chain.** The shape depends on `--workflow`:

  **Default (classic — D21): ONE background headless CLI session.** `Bash run_in_background`, cwd = this main worktree; NOT an Agent-tool subagent — the dispatcher's §5.3 fan-out needs a top-level session (see guardrails): `claude -p --permission-mode bypassPermissions "<chain prompt>"`, whose prompt runs, in order:
  ...
  **`--workflow` (D22) opt-in: two phases; the dispatch fan-out is owned by THIS session for live `/workflows` visibility.**
...
- **Leg 1 invocation decision (D22, 2026-06-11 — supersedes D21 for the `--workflow` opt-in).** **Default stays classic** `/ggx-dispatcher` chained in the headless session — proven, zero-risk; D21's reasoning still holds for the default path. The **`--workflow` opt-in is now unblocked** because D21's two blockers cleared: (1) the §5.2 silent-stall risk (P2) is hard-aborted **before any ticket is locked** by `ggx-dispatcher.md` Step 1.7 (GGC-20); (2) the verify-agent level-2 deadlock is solved by the R4 headless auditor on **both** paths (GGC-19 / PR #81). Phase C is NOT required — an explicit flag ≠ flipping the default. Keeping it opt-in leaves the daily-driver path carrying zero risk while the live-observability path proves out. **Promote `--workflow` to the default only after live validation confirms:** (a) on-duty context stays lean across an 8h run with the workflow tree owned in-session; (b) the inline Phase-B Step 1-4 launch never stalls the wake cadence.
