...
- **Never inline-drive blocking commands.** A blocking fan-out belongs in a top-level `Workflow` task the on-duty session owns — script-spawned agents are level-1, so there is no nested-spawn wall and no need for a headless child. **D22 (default since 2026-06-12):** Leg 1's dispatch is a top-level `Workflow` task owned by the on-duty session; the only inline work is the dispatcher's bounded Step 1-4 launch (not the join — see Leg 1), and worker contexts never flow back (only the `{counts, rows}` return). **GGC-92 extends D22 to Leg 2:** `/ggx-pr-resolver --batch` now fans out via `workflows/pr-resolver-batch.workflow.js` (level-1 script-spawned agents), so Leg 2 likewise runs as a top-level Workflow task on-duty owns — the old headless `claude -p` child that existed ONLY to dodge nested-spawn is gone. The analyze tick (Leg 1a) still runs in a headless child — that is for context separation (keeping the analyzer's per-ticket sweep out of the loop session), unrelated to nested-spawn. Historical note: nested `Agent`/`Task` spawns from inside a subagent FAIL (confirmed live 2026-06-05 — a chain wrapped in an Agent subagent batch-aborted with "spawn tool unavailable"); the Workflow fan-out sidesteps this entirely.
...
> **Cadence (GGC-70):** ~4h, deliberately slow. The resolver does actionable work only ~1-2×/day, so a tight poll just burns resolver-batch Workflow runs (each runs tests) on an empty queue. A few-hour latency on PR-comment resolution is fine; ~4h catches the real passes the same working day. (The cheap in-session CI check in step 1 still runs every Leg-2 tick, so red-CI alerts are not delayed beyond this cadence either — if faster CI alerting is ever needed, that is the lever to split, not the resolver.)
...
**In-session vs spawned (so the wake cycle stays ~1-2 min)**: step 1 (CI check) runs IN-SESSION — it is pure `gh` reads + dedup bookkeeping, no blocking work. Step 2 (resolve) now **owns the batch `Workflow` as a top-level task** (GGC-92, D22-style — exactly like Leg 1b owns the dispatch Workflow): since `/ggx-pr-resolver --batch` fans out via `workflows/pr-resolver-batch.workflow.js` (script-spawned level-1 agents — no nested-spawn wall), and on-duty is itself top-level, **no headless `claude -p` child is needed**. Only the bounded top-level sweep (`gh pr list` + skip-set + pre-spawn filters that build the roster) runs inline; the per-PR units run inside the Workflow task on-duty owns. on-duty records the Workflow `task_id` and folds its completion into a later cycle's summary (the heavy per-PR work never flows back into this session).
