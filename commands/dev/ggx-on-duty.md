...
- `chain.running` → the chain is in flight; **skip starting a new one.** On the completion notification, branch on `chain.phase`:
  - `"dispatch"` → the whole chain is done: fold the `{counts, rows}` return value into the next summary, then set `chain.next_due = now + 2h`, `chain.running=false`, `chain.phase=null`.
