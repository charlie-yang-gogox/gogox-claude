...
- `model`: `"opus"` — required because every pipeline that runs through
  ...
  > **Caveat (2026-06-06, unverified)**: official Claude Code docs
  > (sub-agents.md) state the `Agent` tool is unavailable in ALL
  > subagents — uniformly, regardless of type/background/model — which
  > contradicts the "nested sonnet works in practice" claim above.
  > Re-verify on the next dev-lane run: if `/port:plan` / `/dev:verify`
  > spawns fail inside workers, those stages need the same inline
  > treatment as the four paths above. Behavior unchanged until
  > confirmed.
