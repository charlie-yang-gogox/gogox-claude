  ...
  try {
    ...
    await agent(
      [
        ...
        `  cycle_time_sec (ONLY when merged: mergedAt−createdAt in seconds, parsing`,
        `    BOTH timestamps as UTC. OMIT this key entirely when the PR is not merged`,
        `    — do NOT compute now−createdAt, which is not a cycle time and mixes local`,
        `    now with a UTC timestamp),`,
        ...
        ``,
        `STEP 4 — run the real scan + post WITH the summary. Run EXACTLY this base`,
        `command:`,
        `python3 ~/.claude/skills/session-metrics/session_metrics.py \\`,
        `  --scan-subagents --ticket-id ${res.ticketId} \\`,
        `  --parent-session ${parentSessionId} \\`,
        ...
        `THEN, ONLY if STEP 3.5 actually created $OUTCOME_FILE, append one more`,
        `argument to that same command:  --outcome-json "$OUTCOME_FILE"  (if STEP`,
        `3.5 was skipped, run the base command as-is — do NOT append it).`,
