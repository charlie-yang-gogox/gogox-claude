...
async function runFallback(res) {
  ...
  await agent(
    [
      ...
      `Via the Linear MCP (find it with ToolSearch — use mcp__claude_ai_Linear__*,`,
      `never mcp__linear-server__*): if no comment containing the marker`,
      `<!-- dispatch-fallback-error --> exists on this ticket yet, post one`,
      `summarizing: "${(res.error || "worker died").slice(0, 200)}".`,
