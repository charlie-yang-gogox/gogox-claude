...
**A. Tool availability — interactive AND headless both supported**

This skill writes to Notion via the **claude.ai OAuth-hosted MCP** (`mcp.notion.com/mcp`).
That MCP exposes high-level Notion-flavored Markdown tools (`notion-update-page`,
`notion-create-pages`, etc.) which the skill uses throughout Step 4.

- **Interactive Claude Code session**: tools surface as `mcp__claude_ai_Notion__notion-*`
  (the user's claude.ai catalog auto-loads them).
- **Headless `claude -p` from launchd**: tools surface as `mcp__notion-hosted__notion-*`,
  loaded from the user-scope MCP entry `notion-hosted` in `~/.claude.json` after a
  one-time `claude mcp add --transport http notion-hosted https://mcp.notion.com/mcp --scope user`.
  OAuth refresh tokens are stored in the macOS Keychain under `Claude Code-credentials`
  and refresh automatically; no browser interaction needed after the initial setup.

Both modes give the SAME high-level tools; the only difference is the tool-name prefix.
The skill should detect at runtime which prefix is present and use whichever is available.

If NEITHER prefix is present (rare — Keychain missing, network down, OAuth revoked):
- Output the table to stdout (4a) and emit `> Notion 寫入未執行 (no Notion MCP found)`.
- SKIP 4b/4d. Do NOT substitute raw REST / API-* tools — see B below.
