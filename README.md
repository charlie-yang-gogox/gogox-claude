...
2. `<repo-root>/.gogox-claude.local.yaml` — **per-machine overrides** (gitignored). Holds user-specific paths like `origin_project_path` (used by `/port:*`). First-run writes from `AskUserQuestion`. Each dev's machine has its own.
...
#### `.gogox-claude.local.yaml` — fields

| Field | Used by | Notes |
|---|---|---|
| `origin_project_path` | `/port:start` | Absolute path to the origin codebase (the project being ported FROM). Supports `~` and `$ENV_VAR` expansion. Validated on first read; if path doesn't exist after expansion, `/port:start` prompts to fix |
