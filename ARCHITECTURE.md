...
| **R5** | Fan-out via Workflow (migration complete) | **Phase A validated (CAF-371, 2026-06-08); Phase B shipped; Phase C COMPLETE (GGC-55, 2026-06-13) — the legacy N×`Agent` `--classic` fan-out is RETIRED.** `/ggx-dispatcher` drives the fan-out via `workflows/ggx-dispatch.workflow.js` as the **only** path (`--workflow` is now a redundant no-op accepted for back-compat). ui-tweak runs in-script (`runUiTweak`) as a SCRIPT-spawned level-1 dual-judge panel — the opus judge spawns cleanly. The `Workflow`-tool-unavailable case is a fail-fast abort (the §5.2 invocation error branch), NOT a fallback. **Reversion is git, not dead code:** `git revert` from tag `pre-classic-removal` restores the dual-path skill if the `Workflow` tool is ever changed/removed upstream. Note: verify-agent (spawned by the worker inside `/dev:verify`) stays level-2 — only what the SCRIPT spawns directly is level-1, so the Workflow fan-out does NOT restore verify-agent independence (R4's headless `claude -p` auditor is the escape hatch there). | `commands/dev/ggx-dispatcher.md` §5.1/§5.2, `commands/design/ui-tweak/audit.md` (SYNC), `workflows/ggx-dispatch.workflow.js` |
