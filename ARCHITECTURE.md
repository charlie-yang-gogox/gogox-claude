...
| **R5** | Migrating fan-out to Workflow | **Phase A landed (opt-in).** `/ggx-dispatcher --workflow` drives the dev/port/bug fan-out via `workflows/ggx-dispatch.workflow.js` instead of N×`Agent`; unset = today's path. ui-tweak stays §5.0-inline (Phase B moves it into the script — where its opus judge becomes a level-1 script-spawned agent, dissolving §5.0). Phase C flips the default and removes §5.0/§6.1. Revisit Phase B/C when: official docs change the nesting limit, sonnet nesting actually breaks, OR the inline design-bug lane grows large enough to bottleneck. | `commands/dev/ggx-dispatcher.md` §5.2, `workflows/ggx-dispatch.workflow.js` |
