...
| **R2** | Surviving level-2 sonnet spawns (`/dev:figma`, `/dev:align`) | Kept as spawns, but each carries a **one-time inline fallback**: on spawn-failure the stage runs the subagent's contract inline in the worker, writing the SAME output files with the SAME status encoding so downstream parsers need zero changes. | `commands/dev/dev/figma.md` Step 4b, `commands/dev/dev/align.md` Step 2b |
| **R3** | Independence-load-bearing auditors | NO inline fallback. `verify-agent` audits code the worker itself wrote in `--auto`, so an inline verify would be the implementer self-auditing — collapsing the decorrelation the stage exists to provide. Spawn-failure stays a BLOCKED hard-fail. (`/port:plan` is excluded for a different reason — see R-below.) | `commands/dev/dev/verify.md` Step 2 |
